	...
	private void generateSurface(World world, Random rand, int chunkX, int chunkZ, Random random, World world1, IChunkProvider chunkGenerator, IChunkProvider chunkProvider) 
	{
		...
		if(b.biomeName.equals("Desert"))
		{
			for(int i = 0; i < 16; i++)
			{
				...
				new WorldGenMinable(ModBlocks.salt.blockID, 16).generate(world, random, randPosX, randPosY, randPosZ);
			}
		}
		...
		if(b.biomeName.equals("Beach"))
		{
			for(int i = 0; i < 32; i++)
			{
				...
				int randPosY = random.nextInt(70);
				int randPosZ = chunkZ + random.nextInt(16) + 8;
				new WorldGenMinable(ModBlocks.salt.blockID, 32).generate(world, random, randPosX, randPosY, randPosZ);
			}
		}
	}
