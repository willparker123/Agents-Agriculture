...
public class WorldGenAppleRedTree extends WorldGenerator
{
    ...
    public WorldGenAppleRedTree(boolean par1)
    {
        this(par1, 4, 0, 0, false);
    }

    public WorldGenAppleRedTree(boolean par1, int par2, int par3, int par4, boolean par5)
    {
        super(par1);
        minTreeHeight = par2;
        metaWood = par3;
    ...
    public boolean generate(World par1World, Random par2Random, int par3, int par4, int par5)
    {
            ...
            for (int l = par3 - byte0; l <= par3 + byte0 && flag; l++)
            {
                for (int j1 = par5 - byte0; j1 <= par5 + byte0 && flag; j1++)
                {
                    if (j >= 0 && j < 256)
                    {
                        ...
                        if (j2 != 0 && j2 != ModCrops.appleRedLeaves.blockID && j2 != Block.grass.blockID && j2 != Block.dirt.blockID && j2 != ModCrops.appleRedLog.blockID && j2 != Block.sand.blockID)
                        {
                            flag = false;
                        }
                    }
        ...
        for (int k1 = (par4 - byte1) + i; k1 <= par4 + i; k1++)
        {
            ...
            for (int l3 = par3 - j3; l3 <= par3 + j3; l3++)
            {
                ...
                for (int l4 = par5 - j3; l4 <= par5 + j3; l4++)
                {
                    ...
                    {
                        setBlockAndMetadata(par1World, l3, k1, l4, ModCrops.appleRedLeaves.blockID, metaLeaves);
                    }
                }
            }
        }
        ...
        for (int l1 = 0; l1 < i; l1++)
        {
            ...
            if (l2 != 0 && l2 != ModCrops.appleRedLeaves.blockID)
            {
                continue;
            }

            setBlockAndMetadata(par1World, par3, par4 + l1, par5, ModCrops.appleRedLog.blockID, metaWood);
        ...
        if (growVines)
        {
            for (int i2 = (par4 - 3) + i; i2 <= par4 + i; i2++)
            {
                ...
                for (int i4 = par3 - k3; i4 <= par3 + k3; i4++)
                {
                    ...
                    {
                        if (par1World.getBlockId(i4, i2, k4) != ModCrops.appleRedLeaves.blockID)
                        {
                            continue;
                        }
