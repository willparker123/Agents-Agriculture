...
public class TileEntityDnaFabricator extends TileEntity implements ISidedInventory
{
	...
	public int furnaceSpeed = 1600; //how many ticks to cook an item
	public int burnTime;
	...
	{
		return this.isInvNameLocalized() ? this.localisedName : "DNA Fabricator";
	}
	...
	public void updateEntity()
	{
		...
		if(!this.worldObj.isRemote)
		{
			...
			{
				flag1 = true;
				BlockDnaFabricator.updateDnaFabricatorBlockState(this.burnTime > 0, this.worldObj, this.xCoord, this.yCoord, this.zCoord);
			}
		}
	...
	private boolean canSmelt()
	{
		...
		{
			ItemStack itemstack = DnaFabricatorRecipes.smelting().getSmeltingResult(this.slots[0]);
	...
	private static int getItemBurnTime(ItemStack itemstack)
	{
		...
		else
		{
			...
			if(item instanceof ItemBlock && Block.blocksList[i] != null)
			{
				...
				if(block == ModFuels.organicfuelBlock)
				{
					return 8000; //ticks it burns for
				}

				if(block == ModFuels.organicfuelBlock2)
				{
					return 32000; //ticks it burns for
				}

				if(block == ModFuels.organicfuelBlock3)
				{
					return 128000; //ticks it burns for
				}
			}

			if (i == ModFuels.organicfuel.itemID) return 800; //ticks it burns for
			if (i == ModFuels.organicfuel2.itemID) return 3200; //ticks it burns for
			if (i == ModFuels.organicfuel3.itemID) return 12800; //ticks it burns for
