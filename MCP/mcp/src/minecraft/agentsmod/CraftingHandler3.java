...
public class CraftingHandler3 implements ICraftingHandler
{
	@Override
	public void onCrafting(EntityPlayer player, ItemStack item, IInventory craftMatrix)
	{
		...
		{               
			if(craftMatrix.getStackInSlot(i) != null) //If there is an item
			{
				...
				if (item.itemID == ModTools.foodgrinder.itemID)
				{
					if(j.getItem() != null && j.getItem() == ModTools.foodgrinder) //If it's a firestone
					{
						ItemStack a = new ItemStack(ModTools.foodgrinder, 2, (j.getItemDamage() + j.getMaxDamage())); //Makes a new itemstack that's been damaged and has 2 items
				...
				else
				{
					if(j.getItem() != null && j.getItem() == ModTools.foodgrinder) //If it's a firestone
					{
						ItemStack a = new ItemStack(ModTools.foodgrinder, 2, (j.getItemDamage() + 1)); //Makes a new itemstack that's been damaged and has 2 items
						if(a.getItemDamage() >= a.getMaxDamage())
						{ //If it is destroyed
							--a.stackSize; //Removes the added item
						}
			...
			/*/
			if(player.inventory.hasItem(ModFood.foodgrinder.itemID)); //If there is an item
			{
				player.inventory.consumeInventoryItem(ModFood.foodgrinder.itemID);
			}
			/*/
		}
