...
public class BlockCropWalnutDiamond extends BlockCrops
{
    @SideOnly(Side.CLIENT)
    private Icon[] iconArray;
    
	protected BlockCropWalnutDiamond(int par1)
	{
		super(par1);
	...
	public boolean onBlockActivated(World par1World, int par2, int par3, int par4, EntityPlayer par5EntityPlayer, int par6, float par7, float par8, float par9)
	{
		...
		{
			par5EntityPlayer.inventory.addItemStackToInventory(new ItemStack(ModFruits.walnutDiamond,3));
		...
		if (par1World.getBlockLightValue(par2, par3 + 1, par4) >= 13)
		{
			...
			if (i < 10)
			{
				float f = getGrowthRate(par1World, par2, par3, par4);
	               ...
	               if (par5Random.nextInt((int)(250000000 / f) + 1) == 0)
				{
                    i++;
	...
	public int idDropped(int par1, Random par2Random, int par3)
    {
        if (par1 >= 10)
        {
            return ModFruits.walnutDiamond.itemID;
        }
        else
        {
            return 0;
