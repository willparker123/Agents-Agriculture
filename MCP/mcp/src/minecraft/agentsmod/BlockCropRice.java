...
public class BlockCropRice extends BlockCrops implements IPlantable
 {
    @SideOnly(Side.CLIENT)
    private Icon[] iconArray;
    
	protected BlockCropRice(int par1)
    {
        super(par1);
       ...
       for (int i2 = par2 - 1; i2 <= par2 + 1; i2++)
        {
            for (int j2 = par4 - 1; j2 <= par4 + 1; j2++)
            {
               ...
               if (k2 == ModCrops.wetClay.blockID)
                {
                    f1 = 1.0F;
    ...
    {
        return par1 == ModCrops.wetClay.blockID;
    }
...
public int quantityDropped(Random par1Random)
{
	...
	public void updateTick(World par1World, int par2, int par3, int par4, Random par5Random)
    {
       ...
       if (par1World.getBlockLightValue(par2, par3 + 1, par4) >= 5)
        {
           ...
           if (i < 7)
            {
                ...
                if (par5Random.nextInt((int)(2.5 / f) + 1) == 0)
                {
                   i++;
